...
<CardGroup cols={3}>
  ...
  <Card title="Available Payout Setup" icon="chart-line">
    Complete the payout setup shown in **Finances**, either BEATPAY or Stripe Connect.
  </Card>
...
Exclusive-sale payouts and subscription contribution payouts are separate earnings streams. They can use different payout routes on the same producer account. For example, **Finances** may show BEATPAY ready for exclusive sales while subscription payouts continue through an active Stripe Connect account.

A producer with BEATPAY but without an active subscription payout route can publish eligible **Exclusive Only** offers. **Standard Licensing** stays unavailable until the subscription payout stream is active.
...
<CardGroup cols={2}>
  ...
  <Card title="Stripe Connect" icon="credit-card">
    Stripe Connect remains active for producers whose **Finances** page shows it as their current payout route.
  </Card>
</CardGroup>
...
1. Open **Finances**
2. Click **Set up BEATPAY** if it appears, or click **Connect Stripe Account Now** when Stripe Connect is shown
3. Provide the bank, identity, country, and tax information requested by the active setup
