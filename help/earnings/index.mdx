...
BeatPass gives producers two main ways to earn: direct track sales and subscription-based payouts. Your **Producer Finances** page shows currency-specific earnings, finance activity, and payout readiness in one place.
...
<Columns cols={2}>
  ...
  <Card title="Producer Finances" icon="chart-bar">
    Use **Producer Finances** to review **Total earned**, **Waiting**, **Sending**, and **Sent** in one currency at a time, then use **Finance activity** for individual records.
  </Card>
</Columns>
...
<Steps>
  <Step title="Set up payouts">
    Open **Backstage** and go to **Finances**, then click **Set up bank payouts**. Choose a bank account currency that your bank can receive and add your bank details once. Producers with an existing Stripe Connect account keep receiving payouts through Stripe until their BEATPAY setup is active.
  </Step>
  ...
  <Step title="Track payout status">
    BeatPass groups finance activity as **Waiting**, **Sending**, **Completed**, **Needs attention**, or **Refunded** so you can see where money is in the process.
  </Step>
...
<Columns cols={2}>
  ...
  <Card title="Multi-Currency Support" icon="chart-line">
    **Producer Finances** keeps currencies separate and adds an **Earnings currency** selector when more than one currency is available.
  </Card>
</Columns>
