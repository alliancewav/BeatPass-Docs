---
title: "Payment Methods"
sidebarTitle: "Overview"
description: "Manage your payment methods on BeatPass. Add, update, or remove credit cards and other payment options."
keywords: ["payment methods", "credit card", "payment", "billing", "card"]
"og:title": "Payment Methods - BeatPass Help"
"og:description": "Learn how to manage your payment methods on BeatPass."
---

Your payment method is used to pay for subscriptions and purchases on BeatPass. Keep it up to date to ensure uninterrupted service.

---

## Payment Method Topics

<CardGroup cols={2}>
  <Card title="Supported Methods" icon="credit-card" href="/help/billing/payment-methods/supported-methods">
    See what payment options are available.
  </Card>
  <Card title="Adding a Payment Method" icon="plus" href="/help/billing/payment-methods/adding-payment-method">
    Add a new card or payment option.
  </Card>
  <Card title="Updating Payment Method" icon="refresh-cw" href="/help/billing/payment-methods/updating-payment-method">
    Change your current payment details.
  </Card>
</CardGroup>

---

## Your Saved Payment Method

When you have a payment method on file, you'll see:

| Information | Description |
|-------------|-------------|
| **Card brand** | Visa, Mastercard, American Express, etc. |
| **Last 4 digits** | The final four numbers of your card |
| **Expiration** | Month and year the card expires |

<Note>
  For security, we never display your full card number. Only the last 4 digits are shown for identification.
</Note>

---

## Where to Manage Payment Methods

To view or update your payment method:

1. Go to **Settings**
2. Select **Subscription**
3. Find the **Payment Method** section

---

## Payment Security

Your payment information is handled securely:

- **Not stored on our servers** — Card details are managed by our payment processor (Stripe)
- **Encrypted transmission** — All payment data is encrypted
- **PCI compliant** — We follow industry security standards

---

## When You Need a Payment Method

A payment method is required for:

- **Subscribing to a paid plan** — To process recurring payments
- **Upgrading your plan** — To charge the price difference
- **Purchasing beats** — For one-time transactions

### Without a Payment Method

If you don't have a payment method saved:

- You can use BeatPass with a free account
- You'll be prompted to add one when subscribing
- Existing subscriptions may fail to renew

---

## Troubleshooting

<AccordionGroup>
  <Accordion title="My payment failed">
    Common reasons include:
    - Card expired
    - Insufficient funds
    - Card issuer declined the charge
    
    Try updating your payment method or contact your bank.
  </Accordion>
  
  <Accordion title="I don't see my payment method">
    If you subscribed via PayPal, your payment method is linked to your PayPal account rather than a card on file.
  </Accordion>
</AccordionGroup>

---

## Related Topics

<CardGroup cols={2}>
  <Card title="Subscription Overview" icon="credit-card" href="/help/billing/subscription-overview">
    View your current subscription status.
  </Card>
  <Card title="Invoices" icon="file-text" href="/help/billing/invoices">
    View your payment history.
  </Card>
</CardGroup>
