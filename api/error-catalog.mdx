...
| Error | Code | Cause | Resolution |
|-------|------|-------|------------|
| `Unauthenticated` | 401 | Missing auth token/cookie | Include valid Authorization header |
| `Token expired` | 401 | Bearer token no longer valid | Obtain new token |
| `Invalid token` | 401 | Malformed or revoked token | Re-authenticate |
...
| Error | Code | Cause | Resolution |
|-------|------|-------|------------|
| `File too large` | 422 | Exceeds max file size | Reduce file size (WAV: 100MB, MP3: 50MB) |
| `Invalid file format` | 422 | Unsupported audio format | Use WAV, MP3, or FLAC |
...
| `Bitrate too low` | 422 | MP3 under 128kbps | Re-export at 320kbps |
...
<Steps>
  ...
  <Step title="Verify Authentication">
    Ensure token is valid and included correctly.
  </Step>
