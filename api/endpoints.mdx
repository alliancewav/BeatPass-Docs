...
Anyone can verify a license using its UUID—no authentication required.
...
- **Albums** – canonical releases created by producers.
- **Liked albums** – a users saved collections in their library.
...
<Tabs>
  <Tab title="track">
    **Track ID** — Returns similar tracks based on audio features, metadata, and listener patterns.
  </Tab>
  <Tab title="artist">
    **Artist ID** — Returns tracks from the specified artist and similar producers in the same genre.
  </Tab>
  <Tab title="genre">
    **Genre slug** — Returns popular and trending tracks across the entire genre.
  </Tab>
</Tabs>
...
| Method | Endpoint                     | Auth | Description                                                              |
| ------ | ---------------------------- | ---- | ------------------------------------------------------------------------ |
| GET    | `/users/{user}/minutes-left` | 🔓   | Check remaining usage minutes (for example, upload or processing budget) |
